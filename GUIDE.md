...
*   `structure.md`: 프로젝트의 파일 구조, 핵심 변수/타입 정의, 모듈 간의 관계
*   `lims.txt`: AI 기능 구현의 절대적인 최우선 기준 및 관련 예시
...
*   **문서 우선 개발**: 코드를 수정하기 전에 관련된 문서(특히 `GEMINI.md`, `GUIDE.md`, `structure.md`)를 먼저 업데이트합니다.
...
### 3.14. 프로젝트 구조 및 핵심 요소 관리

**표준 가이드**: 프로젝트의 구조, 핵심 변수 및 타입 정의, 모듈 간의 관계는 `structure.md` 파일에 명확하게 문서화되어야 합니다. 이는 코드의 이해도를 높이고, 변경 사항의 영향을 파악하며, 일관된 개발을 유지하는 데 필수적입니다.

**적용법**:
*   **`structure.md` 업데이트**: 새로운 기능 추가, DB 스키마 변경, 핵심 로직 리팩토링 등 프로젝트 구조에 영향을 미치는 주요 변경이 있을 때마다 `structure.md`를 업데이트합니다.
*   **타입 정의 일관성**: TypeScript 타입 정의를 중심으로 변수 및 데이터 흐름의 일관성을 유지합니다. `structure.md`에 정의된 핵심 변수 및 타입 정의를 따릅니다.
*   **변수/타입 변경 시 지침**: 변수나 타입 정의가 변경될 경우, 해당 변경이 영향을 미치는 모든 파일과 `structure.md`를 업데이트합니다. 특히, `GUIDE.md`의 `3.1. 일반 코딩 스타일` 섹션의 `TypeScript 사용` 지침을 준수합니다.

**예시 프롬프트**:
```
"`prisma/schema.prisma`에 새로운 `Team` 모델을 추가했습니다. 이 변경 사항을 `structure.md`에 반영하고, `Team` 모델과 관련된 핵심 변수 및 타입 정의를 추가해주세요."
```
